...
bool RH_E32::readParameters(Parameters& params)
{
  ...
  Serial.print("result readBytes: ");	Serial.println(result);
  Serial.print("sizeof(params) ");	Serial.println(sizeof(params));
  
  Serial.println("Received Parameters:");
   Serial.print("head: 0x");
  Serial.println(params.head, HEX);
   Serial.print("addh: 0x");
  Serial.println(params.addh, HEX);
   Serial.print("addl: 0x");
  Serial.println(params.addl, HEX);
   Serial.print("sped: 0x");
  Serial.println(params.sped, HEX);
   Serial.print("chan: 0x");
  Serial.println(params.chan, HEX);
   Serial.print("option: 0x");
  Serial.println(params.option, HEX);
...
bool RH_E32::writeParameters(Parameters& params, bool save)
{
  ...
  Serial.print("sizeof(params) ");	Serial.println(sizeof(params));
	...
	Serial.println("Received Parameters:");
	// Serial.print("head: 0x");
	Serial.println(params.head);
	// Serial.print("addh: 0x");
	Serial.println(params.addh);
	// Serial.print("addl: 0x");
	Serial.println(params.addl);
	// Serial.print("sped: 0x");
	Serial.println(params.sped);
	// Serial.print("chan: 0x");
	Serial.println(params.chan);
	// Serial.print("option: 0x");
	Serial.println(params.option);

  
  if (result != sizeof(params))
  {
	  Serial.println("result != sizeof(params)");
    return false;
  }
...
bool RH_E32::getVersion()
{
	...
	if(version[0] != RH_E32_COMMAND_READ_VERSION) // first byte must be 0xc3
	{
		Serial.println("Not a E32");
		return false;
	}
/*  
	Serial.println();
	...
	if (result == sizeof(version)) {
	  // Iterate through the array and print each byte in hex format
	  for (int i = 0; i < sizeof(version); i++) {
		Serial.print("0x");
		// Print leading 0 for single digit hex values
		if (version[i] < 0x10) Serial.print("0");
		Serial.print(version[i], HEX); // Print the hex value
		if (i < sizeof(version) - 1) {
		  Serial.print(", "); // Print comma between bytes
		}
	  }
